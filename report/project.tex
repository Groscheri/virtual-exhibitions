...
\end{figure}
  ...
  ZEUS was built in order to help developers to create multi-agent system with higher level 
  tools. ZEUS architecture consists of different components fully written in Java so that it 
  takes all the cross and multi-platform advantages of Java programming language. There are 
  ...
  developers to concentrate on the content and the main goal of their agents and not on the way to make 
  ...
  pause, resume or stop them in a consistent way).
  ...
  debugging tools (which is not provided by JADE framework). It also embeds useful services like a nameserver 
  and a facilitator agent (which acts in the same way as the one in JADE) to provide agents and services 
  discovery.
  ...
  Due to the lack of documentation and papers around ZEUS, we were not able to find any notable projects 
  ...
  because it is a bit outdated and documentation is harsh to find. Indeed, documentation 
  around ZEUS framework is really thin and during this task, it was complicated to 
  find relevant information or even to verify our sources.
  
  ZEUS framework would have been really interesting to use with the debug tool it provides 
  since this is not a feature provided by JADE framework. It can be very useful in order to 
  understand how the system is running and even more if it is complex.
