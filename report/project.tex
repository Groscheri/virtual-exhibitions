...
\end{figure}
  ...
  Like JADE, Jack Intelligent Agents (JACK) framework is a Java developed tool to build multi-agent 
  systems. It was created in 1997 by AOS\footnote{Agent Oriented 
  Software Pty. Ltd.} company, it is proprietary and it is still maintained 
  and developed.
  ...
  different components to deal with such a model:
  \begin{itemize}
    \itemsep0pt
    ...
    \item \textbf{JACK Agent Kernel} which is an engine that runs JACK Agent language 
    sources. It also provides all the libraries and classes needed to develop multi-agent 
    systems
  \end{itemize}

  \textbf{JACK Agent Language} is built on top of Java language as a plugin. It includes 
  several operators to deal with agent development. Thanks to \textbf{JACK Agent Compiler} 
  and \textbf{JACK Agent Kernel}, it is possible to translate sources from this language 
  into pure Java and execute it.
  
  The core framework doesn't provide any FIPA implementations. Moreover, there isn't any 
  ...
  Several simulation applications like human behavior, autonomous systems, decision 
  ...
  experience and more tools. Furthermore, relying on proprietary software could bring serious 
  issues the day when the software is not maintained anymore.
